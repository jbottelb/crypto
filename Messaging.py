'''
Class for handling messaging in the system
'''
...
class Messaging:
    ...
    def read_message(sock, connections):
        '''
        ...
        try:
            ...
            if Messages._isValidMessage(message):
    ...
    def get_neighbors(addr):
        ...
        try:
            ...
            return None
        rc = Messages.send_message(message, True, sock=sock)
        if rc:
            # sent message successfully to trusted node,
            # read response
            response = Messages.readMessage(sock)
            if response is not None:
                returnList = response.get("Neighbors", None)
        sock.close()
        return returnList

    def send_message(message, keepSocketOpen = False, addr = None, sock = None, connections = None):
        '''
    ...
    def get_blockchain(neighbor):
        ...
        rc = Messages.send_message(message, True, neighbor, sock=sock, connections=dict())
        if not rc:
        ...
        while blocks_left_to_come:
            ...
            for txn in transactions_dicts:
                txn_object = Messages.transactionDictToObject(txn)
