...
class Messaging:
    ...
    @staticmethod
    def readMessage(sock, connections = None):
        '''
        ...
        try:
            ...
            if Messaging._isValidMessage(message):
    ...
    def getActiveSeedNodes():
        ...
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            if not Messaging._connectToNameServer(sock):
        ...
        if not Messaging.sendMessage({"Type": MessageTypes.Get_Seed_Nodes}, True, sock=sock):
    ...
    def getNeighbors(addr):
        ...
        try:
            ...
            return None
        rc = Messaging.sendMessage(message, True, sock=sock)
        if rc:
    ...
    @staticmethod
    def sendMessage(message, keepSocketOpen = False, addr = None,
                    sock = None, connections = None):
        '''
    ...
    @staticmethod
    def transactionDictToObject(txn_dict):
        '''
    ...
    def getBlockchain(neighbor):
        ...
        rc = Messaging.sendMessage(message, True, neighbor, sock=sock, connections=dict())
        if not rc:
        ...
        while blocks_left_to_come:
            ...
            for txn in transactions_dicts:
                txn_object = Messaging.transactionDictToObject(txn)
